...
const Post = ({ data }) => {
  ...
  const handleDeletePost = useCallback(
    (post_id, history) => {
      const url =
        "http://ec2-3-38-101-203.ap-northeast-2.compute.amazonaws.com/content/delete";
      const payload = {
        id: post_id,
      };
      axios
  ...
  return (
    <>
      <PostContainer>
        ...
        <PostHeader>
          ...
          {userName === data.writer ? (
            <ButtonDiv>
              <Button onClick={() => history.push(`/modify/${data.id}`)}>
                수정
              </Button>
              <Button onClick={() => handleDeletePost(data.id, history)}>
                삭제
              </Button>
            </ButtonDiv>
          ) : null}
      ...
      <AllCommentsSection>
        ...
        {userName.length > 0 ? (
          <WriteComment post_id={data.id} userName={userName} />
        ) : null}
      </AllCommentsSection>
    </>
  );
