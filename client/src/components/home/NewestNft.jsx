...
const fetchNewestNft = async () => {
  const getPost = await axios.get(
    `http://ec2-3-38-101-203.ap-northeast-2.compute.amazonaws.com/home/newstnft`
  );
...
const NewestNft = () => {
  ...
  useEffect(() => {
    getMetaData().then((el) => setMetaData(el));
  }, []);
        ...
        {metaData.length > 0
          ? metaData.map((el) => {
              return (
                <ImageDiv
                  key={el.nft.id}
                  onClick={() => handleClick(el.nft.id, el.nft, el.metadata)}
                >
