...
const fetchNewestPost = async () => {
  const getPost = await axios.get(
    `http://ec2-3-38-101-203.ap-northeast-2.compute.amazonaws.com/home/newstpost`
  );
      ...
      {newestPost.title ? (
        <>
          <ContainerHeader>
            ...
            <Shortcuts>
              <Link to={`/postdetail/${newestPost.id}`}>바로 가기</Link>
            </Shortcuts>
          </ContainerHeader>
          <NewestPostDiv>
            <PostHeaderDiv>
              <span style={{ padding: "1rem", fontSize: "1.2rem" }}>
                {newestPost.title}
              </span>
              <span style={{ padding: "1rem" }}>
                작성자 : {newestPost.writer}
              </span>
            </PostHeaderDiv>
